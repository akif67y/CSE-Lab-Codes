...
// =========================================================
//            CLASS: CUSTOM PROBING HASH TABLE
// =========================================================
...
{
  public:
    // hashId: 1 for Polynomial, 2 for FNV
    // C1, C2: User defined constants for the probing formula
...
// Used for remove/contains logic (does not count collisions)
template <typename HashedObj>
int CustomHashTable<HashedObj>::findPos( const HashedObj & x ) const
{
           ...
           array[ currentPos ].element != x )
    {
        ...
        // FORMULA: (Hash(k) + C1 * i * auxHash(k) + C2 * i^2) % N
...
// --- INSERT WITH CUSTOM PROBING AND COLLISION COUNTING ---
template <typename HashedObj>
bool CustomHashTable<HashedObj>::insert( const HashedObj & x )
{
    int tableSize = array.size();
    
    // 1. Calculate Base Hash Values
    long long hashVal = (hashFunctionId == 1) ? 
                        hash1_polynomial(x, tableSize) : 
                        hash2_fnv(x, tableSize);
    long long aux = auxHash(x, tableSize);
    ...
    // 2. Probing Loop
    // Continues while slot is occupied by a DIFFERENT element
    while( array[currentPos].info != EMPTY && array[currentPos].element != x ) {
        
        totalCollisions++; // Count every iteration as a collision
...
void CustomHashTable<HashedObj>::rehash( int newSize )
{
    ...
    // Note: Re-inserting recalculates positions using the new size N
...
int main() {
    ...
    // User Defined Constants for Custom Probing
    const int C1 = 13;
    const int C2 = 11;
