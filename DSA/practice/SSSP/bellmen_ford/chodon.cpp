...
bool solve(){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < i; j++){
            ...
            int weight = stoi(st);
            edge_list.push_back({weight, {i+1, j+1}});
            edge_list.push_back({weight, {j+1,i+1}});
        }
    }
    for(int i = 1; i <= n; i++){
        edge_list.push_back({0,{i,i}});
    }
    ...
    debug(distances);
    ll max_verstappen = -INF;
    for(int i = 1; i <= n; i++){
        if(distances[i] > max_verstappen){
            max_verstappen = distances[i];
        }
    }
    cout << max_verstappen<<"\n";
    //   for(int i = 0; i < n; i++){
    ...
    //         if(reachable_neg[from]){
    //             reachable_neg[to] = true;
    //         }
