...
void solve(){
  ...
  //  vector<int> parents(n+1, -1);
    vector<vector<ll>>answer(n+1, vector<ll>(3));
    for(auto &var : answer){
        var[0] = 0; // count
        var[1] = INF; //minimum edge
        var[2] = -INF; // maximum edge
    }
    ...
    while(!pq.empty()){
        ...
        for(auto& edge : graph[u]){
            ...
            if(distances[u] + weight < distances[v]){
                ...
                answer[v][0] = answer[u][0];
                answer[v][1] = answer[u][1] +1;
                answer[v][2] = answer[u][2]+1;
                pq.push({distances[v], v});
            }
            else if(distances[u] + weight == distances[v]){
                answer[v][0] = (answer[v][0] + answer[u][0]) % M;
                answer[v][1] = min(answer[v][1], answer[u][1] +1);
                answer[v][2] = max(answer[v][2],answer[u][2]+1);
            }
        }
    }
