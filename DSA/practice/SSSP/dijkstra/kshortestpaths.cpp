...
void solve(){
    ...
    for(ll i = 0; i < m; i++){
       ...
       // graph[b].push_back({c, a});  //undirected and {weight, node}
    }

    priority_queue<pair<ll,ll>, vector<pair<ll,ll>>, greater<pair<ll,ll>>> pq; //min priority queue

    vector<vector<ll>> distances(n+1, vector<ll>(k , INF));  //keep track of distancess
   // vector<ll> parents(n+1, -1);
    vector<ll>count(n+1,0); //how many times been extracted
    
    distances[s][0] = 0; // [s][0] = s er first shortest path
    debug(distances); //check the array
    pq.push({0, s});
    ...
    while(!pq.empty()){
        ...
        for(auto& edge : graph[u]){
            ...
            if(count[v] < k){
                pq.push({d + weight, v}); //relaxes all the edges
            }
        }
    }
    debug(distances); 

    for(auto &var : distances[n]){
        cout << var <<" ";
    }
